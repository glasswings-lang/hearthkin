    ...
    def __init__(self, parent, kin, heartbeat=None):
        ...
        info = wx.TextCtrl(
            ...
            value=(
                ...
                "silence sends nothing. If it writes something but doesn't "
                "send it, it is asked once whether it meant to, and its answer "
                "is respected. Words it was never asked about are kept in "
                "logs\\heartbeat_unsent.log so they aren't lost. It only runs "
                "while Hearthkin is open.\n\n"
                "Turning it on here is all it takes — the kin's ability to "
                "reach out is part of this feature; there's no separate tool "
                "to enable.\n\n"
                "About cost: each heartbeat runs the model at least once even "
                "when the kin decides to stay quiet, and twice when it is "
                "asked about something it wrote. On a kin that runs on your "
                "own machine that's free; on a kin using a paid online "
                "provider, keep the interval long so quiet check-ins don't "
                "add up."
            ),
        )
