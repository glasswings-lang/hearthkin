...
class ToolSettingsDialog(wx.Dialog):
    def __init__(self, parent, cfg, save_param, save_telegram_param, kin_name=""):
        ...
        help("Affects REMOTE surfaces only (Telegram, Discord) — the desktop "
             ...
             "is refused. The same limit applies on Telegram when the kin "
             "saves a file by writing it out in a fenced block in its reply. "
             "ON: the kin reads and writes anywhere on this "
             "machine over Telegram/Discord, exactly as it already can on the "
             "desktop, fenced-block saves included. This is a separate switch "
             "from the exec one above — "
